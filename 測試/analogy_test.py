...
class NGramRNN(torch.nn.Module):
	...
	def generate(self,inputs):
		with torch.no_grad():
			...
			_,RNN_out=self.rnn_layer(embed)
			return RNN_out
def syntatic_and_semantic_test(model):
	...
	for i in test_data:
		if i[0].isalpha():
			test_word=re.split(' |\t',i)
			with torch.no_grad():
				embed_words=[model.get_embed(i).view(-1) for i in test_word]
				similarity=torch.cosine_similarity(embed_words[1]-embed_wrods[0]+embed_words[2],embed_words[3])
				cos_sum+=similarity.tolist()[0]
		else:
			print('cosine_similarity_sum:',cos_sum)
			print(i)
...
syntatic_and_semantic_test(model)